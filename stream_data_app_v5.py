...
# Fetch game ball data
@st.cache_data
def fetch_game_balls(token, session_id):
    headers = {
        "Authorization": f"Bearer {token}",
        "accept": "application/json"
    }
...
# Fetch game plays
@st.cache_data
def fetch_game_plays(token, session_id):
    headers = {
        "Authorization": f"Bearer {token}",
        "accept": "application/json"
    }
    ...
    if response.status_code == 200:
        ...
        st.error(f"Game plays fetch error: {response.status_code} - {response.text}")
        return pd.DataFrame()
...
if date_from and date_to:
    ...
    if access_token:
        ...
        if not adhoc_sessions_df.empty:
            ...
            balls_df = fetch_game_balls(access_token, chosen_session_id)
            plays_df = fetch_game_plays(access_token, chosen_session_id)

            if not plays_df.empty and not balls_df.empty:
                st.subheader("Game Plays")
                st.dataframe(plays_df)

                st.subheader("Game Balls")
                st.dataframe(balls_df)

                # Rename for join
                balls_df = balls_df.rename(columns={"playId": "playID"})
                merged_df = pd.merge(plays_df, balls_df, how="left", on="playID")
                ...
                st.download_button("Download Merged CSV", filtered_df.to_csv(index=False), "merged_filtered.csv", "text/csv")
            else:
                st.warning("Plays or ball data not available for this session.")
        else:
            st.warning("No Adhoc sessions found in selected date range.")
